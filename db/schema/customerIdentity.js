...
const { customerIdentityProviderEnum } = require("./enums");
const { customers } = require("./customer");

// One customer, many ways to sign in. The phone identity lives on the
// customers row itself (unique phone + phone_verified_at); everything else —
// email+password, Google, Apple, PIN — is a row here. Passkeys get their own
// table below because one customer holds many credentials.
//
...
const customerIdentities = pgTable(
  "customer_identities",
  {
...
// SHA-256 lands here.
const customerTrustedDevices = pgTable(
...
// password manager.
const customerPasskeys = pgTable(
  "customer_passkeys",
  {
    id: serial("id").primaryKey(),
    customerId: integer("customer_id")
...
const webauthnChallenges = pgTable(
  "webauthn_challenges",
  {
    id: serial("id").primaryKey(),
