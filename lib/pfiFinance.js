 ...
 *
 * These are consumer_order's real raw status values, not the capitalised
 * Sman-vocabulary names (see utils/orderStatusMapping.js) — Loading and
 * Completed are both the single raw value 'loaded' there (distinguished only
 * by releaseStatus, which this flat revenue check doesn't need). Comparing
 * against the capitalised names here meant `status = ANY(...)` matched
 * nothing, ever — every PFI's revenue and order count read zero regardless
 * of how much was actually sold against it.
 */
const REVENUE_STATUSES = ["paid", "released", "loaded"];
...
function computeFinancials(pfi, agg = {}) {
  ...
  // Sold comes from the movement ledger, never from order status — a release
  // is a physical event and the ledger is the only record of it.
  const movementQty = num(agg.movementQty) ?? 0;
  const allocationQty = num(agg.allocationQty) ?? 0;
  const sold = movementQty + allocationQty;
  ...
  // Per-litre cost against the tank quantity, not the BL quantity — a
  // discharge shortage correctly pushes this up, because you paid the same
  // grand total for less product than the papers said would arrive.
